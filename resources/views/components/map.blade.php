         ...
         <div x-cloak x-show="legendOpened" x-transition:enter="transition-opacity duration-300"
                ...
                <div class="absolute inset-1 rounded-md bg-white bg-opacity-75 p-2">
                    <div class="flex items-start justify-between">
                        <h3 class="text-lg font-medium text-slate-700">Manandona</h3>
                            ...
                            <li class="flex items-center px-2 py-1">
                                <div x-id="['legend-checkbox']">
                                    <label x-bind:for="$id('legend-checkbox')" class="flex items-center">
                                        <input type="checkbox" x-bind:checked="layer.getVisible()" x-bind:id="$id('legend-checkbox')"
                                    ...
                                    <template x-if="layer.get('label') === 'Agriculture' && layer.getVisible()">
                                        <div class="mt-2 ml-6 text-sm text-slate-600">
                                            <template x-for="(feature, index) in manadonasFeatures" :key="index">
                                                <a href="#" :title="'Go to ' +  feature.get('name')" x-text="feature.get('name')"
